package fr.raphkitue.bussy

import com.intellij.psi.*
import com.intellij.psi.search.GlobalSearchScope
import fr.raphkitue.bussy.util.BusLog
...
object PsiUtils {
    ...
    fun isEventBusReceiver(psiElement: PsiElement): Boolean {
        if (psiElement is PsiMethod) {
            if (psiElement.annotations.any { it.qualifiedName == "com.google.common.eventbus.Subscribe" }) {
                return true
            }
        }
    ...
    fun isSuperOfEventBus(psiClass: PsiClass): Boolean {
        ...
        for (superClass in supers) {
            try {
                if (superClass.qualifiedName == "tv.make.playout.event.bus.ScheduleEventBus") {
                    return true
                }
            } catch (e: Exception) {
                println(e.toString())
